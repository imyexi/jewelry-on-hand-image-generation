...
def test_ring_generation_write_requires_confirmation_snapshot(tmp_path):
    paths = RunPaths.create(tmp_path, "ring-missing-snapshot")
    write_json(paths.analysis_dir / "product_analysis.json", _ring_analysis_data())
...
def test_ring_generation_rejects_snapshot_ring_field_mismatch(
    ...
    paths = RunPaths.create(tmp_path, f"ring-mismatch-{field_name}")
    write_json(paths.analysis_dir / "product_analysis.json", _ring_analysis_data())
    snapshot = _ring_confirmation_snapshot(**{field_name: value})
...
def test_ring_generation_rejects_incomplete_confirmation_snapshot(
    ...
    paths = RunPaths.create(tmp_path, f"ring-missing-{missing_field}")
    write_json(paths.analysis_dir / "product_analysis.json", _ring_analysis_data())
    snapshot = _ring_confirmation_snapshot()
    del snapshot[missing_field]

    with pytest.raises(ReviewGateError, match=missing_field):
        write_review_decision(
            paths,
            {
                "action": "generate_rank_1",
...
def test_ring_generation_accepts_matching_confirmation_snapshot(tmp_path):
    paths = RunPaths.create(tmp_path, "ring-valid-snapshot")
    write_json(paths.analysis_dir / "product_analysis.json", _ring_analysis_data())
    ...
    assert read_json(decision_path)["confirmation_snapshot"] == (
        _ring_confirmation_snapshot()
    )
...
def test_review_bundle_rejects_bracelet_semantics_imported_into_necklace(
    ...
    with pytest.raises(ReviewGateError, match="项链.*手串语义"):
        review_decision.write_review_bundle(
            paths,
            {
                "action": "generate_rank_1",
...
def test_review_bundle_same_category_correction_preserves_corrected_must_keep(
    ...
    corrected_item = {
        ...
        "location": "第二层正面中央",
        "visual_shape": "透明水滴形且尖端向下",
        "relationship": "连接第 2 层链条",
        "forbid": ["换层", "翻面", "复制"],
        "qc_question": "第二层水滴吊坠是否保持人工确认的形状和连接？",
    }
    constraints["must_keep"] = [corrected_item]
    ...
    assert canonical["must_keep"] == [corrected_item]
...
@pytest.mark.parametrize(
    ...
    [
        (_necklace_analysis_data(), "主珠和配珠保持手腕环绕", "项链.*手串语义"),
        (
            {
                "product_type": "手串",
                "wear_position": "手腕",
...
def test_review_bundle_rejects_ring_constraints_not_traceable_to_final_analysis_before_replace(
    ...
    with pytest.raises(ReviewGateError, match="戒指产品保真约束"):
        review_decision.write_review_bundle(
            paths,
            {
                "action": "generate_rank_1",
