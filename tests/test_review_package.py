...
def ring_analysis_data(**overrides):
    data = {
        ...
        "visible_appearance": "单枚银色戒指，正面戒面清晰",
        "color_family": ["银色"],
        "style_mood": "简洁",
        "composition": "手部近景",
        "product_dimensions": {},
...
def test_write_review_package_copies_selected_references_and_writes_candidates(tmp_path):
    ...
    assert ReferenceRow.from_dict(selected_json[0]["metadata"]) == selected[0].row
    assert selected_json[0]["metadata"]["file_exists"] is True
    assert selected_json[0]["metadata"]["default_strategy"] == "常规可优先使用"
    assert selected_json[0]["metadata"]["bracelet_applicability"] == "是"
    expected_sha256 = hashlib.sha256(b"ref").hexdigest()
    assert selected_json[0]["metadata"]["source_sha256"] == expected_sha256
    assert selected_json[0]["metadata"]["review_sha256"] == expected_sha256
...
def test_review_page_displays_ring_confirmation_and_reference_fields(tmp_path):
    paths = RunPaths.create(tmp_path, "ring-review")
    product = paths.input_dir / "product-on-hand.jpg"
    product.write_bytes(b"ring-product")
    write_json(paths.analysis_dir / "product_analysis.json", ring_analysis_data())
    selected = [
        make_scored(
            tmp_path,
            reference_fields={
    ...
    html = write_review_package(paths, product, selected, selected).read_text(
        encoding="utf-8"
    )
