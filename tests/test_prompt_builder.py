...
def test_严格v2吊坠提示词的权威身份不包含冲突自由文本():
    product = _necklace_product(
        ProductType.PENDANT_NECKLACE,
        visible_appearance="双层项链，可见连接环连接第二层链条",
        layer_count=2,
        pendant_layer=1,
    ...
    prompt = build_generation_prompt(
        product,
        _scored(_row(jewelry_type="带链吊坠")),
        constraints,
        OutputRole.LIFESTYLE,
        _snapshot_for_product(product),
    )

    identity = json.loads(_json_payload(prompt, PRODUCT_IDENTITY_JSON_PREFIX))
    canonical = json.loads(_json_payload(prompt, CANONICAL_CONSTRAINTS_JSON_PREFIX))
    authoritative_text = json.dumps(
        {"产品身份": identity, "保真约束": canonical},
        ensure_ascii=False,
    )

    assert "visible_appearance" not in identity
    assert identity["pendant_layer"] == 1
    assert identity["connection_structure"] == "闭口金属扣连接第一层链条"
    assert "第一层" in authoritative_text
    assert "闭口金属扣" in authoritative_text
    assert "第二层" not in authoritative_text
    assert "连接环" not in authoritative_text
