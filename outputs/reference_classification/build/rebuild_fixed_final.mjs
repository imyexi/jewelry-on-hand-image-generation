...
function classify(item, idx0) {
  ...
  if (n <= 49) {
    purpose = '上手姿势/手模构图参考';
    product = '手链/手串';
    recommendation = '适合生成前期参考手势、手臂角度、皮肤光感与留白构图；可用于手链/手串上手参考';
    ...
    notes='主要看手部姿态；手腕/前臂露出面积足，可用于手链/手串上手参考';
    confidence='高';
  } else if (n <= 55) {
  ...
  const largeWristRanges = [[1,49],[66,75],[77,81],[85,86],[88,91],[94,101],[115,123],[131,133],[137,138],[141,146],[149,165],[173,176],[178,178],[186,193],[196,201],[204,206]];
  const isLargeWrist = largeWristRanges.some(([a,b]) => n >= a && n <= b);
  braceletUse = isLargeWrist ? '是：手腕/前臂露出面积足，可用于手链/手串上手参考' : '否/弱：手腕露出不足或主体不是手腕';
  if (isLargeWrist && !/手链|手串/.test(product)) product = product && product !== '无明确饰品/弱饰品' ? `${product}；手链/手串` : '手链/手串';
  const lowPriorityStyles = ['节庆童趣/端午国风', '新中式/复古手作/银发人群', '韩系少女/街拍咖啡馆'];
  const usePolicy = lowPriorityStyles.includes(style) ? '无特殊要求不优先使用' : '常规可优先使用';
  return {序号:n, 文件名:item.name, 相对路径:item.rel, 绝对路径:item.path, 宽度:item.w||'', 高度:item.h||'', 大小MB:+(item.bytes/1024/1024).toFixed(2), 用途分类:purpose, 手链手串适用性:braceletUse, 默认使用策略:usePolicy, 风格分类:style, 场景关键词:scene, 饰品类型:product, 推荐使用方式:recommendation, 备注:notes, 判断置信度:confidence};
}
...
detail.getRangeByIndexes(0,0,rows.length+1,headers.length).format = { font:{name:'Microsoft YaHei', size:10}, borders:{preset:'all',style:'thin',color:'#E5E7EB'} };
...
[52,210,330,520,58,58,70,170,260,170,170,210,150,310,190,90].forEach((w,c)=>detail.getRangeByIndexes(0,c,rows.length+1,1).format.columnWidthPx=w);
detail.getRangeByIndexes(1,7,rows.length,9).format.wrapText = true;
try { detail.tables.add(`A1:P${rows.length+1}`, true, 'ReferenceClassificationFixed'); } catch(e) {}
...
summary.getRange('A1').values = [['参考图用途与风格分类汇总（修正版）']];
...
summary.getRange('D3:E3').values = [['修正说明', '手腕/前臂大面积露出 => 饰品类型写入手链/手串']];
summary.getRange('A5:B5').values = [['用途分类','数量']];
summary.getRangeByIndexes(5,0,countsBy('用途分类').length,2).values = countsBy('用途分类');
summary.getRange('D5:E5').values = [['风格分类','数量']];
summary.getRangeByIndexes(5,3,countsBy('风格分类').length,2).values = countsBy('风格分类');
summary.getRange('G5:H5').values = [['饰品类型','数量']];
summary.getRangeByIndexes(5,6,countsBy('饰品类型').length,2).values = countsBy('饰品类型');
...
[190,70,30,260,160,30,260,70].forEach((w,c)=>summary.getRangeByIndexes(0,c,30,1).format.columnWidthPx=w);
summary.getRange('A32:H36').merge();
summary.getRange('A32').values = [['说明：已把 1 (1).png 这类“手腕/前臂大面积露出但未佩戴饰品”的参考图，直接在“饰品类型”列写入“手链/手串”。并新增“默认使用策略”：节日、童趣、老人、街拍咖啡馆风格若无特殊要求，则不优先使用。']];
...
[150,90,560,180,140].forEach((w,c)=>index.getRangeByIndexes(0,c,idxRows.length+1,1).format.columnWidthPx=w);
try { index.tables.add(`A1:E${idxRows.length+1}`, true, 'PreviewIndexFixed'); } catch(e) {}

const fixedFile = path.join(outDir, '参考图用途风格分类表_修正版_v2.xlsx');
const xlsx = await SpreadsheetFile.exportXlsx(wb);
await xlsx.save(fixedFile);
// 原文件可能正被 Excel/WPS 打开而锁定，先只保存新修正版。
const imported = await SpreadsheetFile.importXlsx((await fs.readFile(fixedFile)).buffer);
const check = await imported.inspect({kind:'table', range:'分类明细!I1:M8', include:'values', tableMaxRows:8, tableMaxCols:5, maxChars:4000});
console.log(check.ndjson);
console.log('SAVED', fixedFile);
